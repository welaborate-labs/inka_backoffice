  ...
  describe 'valitations' do
    ...
    describe "should verify the 'length'" do
      ...
      it "should verify the 'maximum'" do
        ...
        user.save
        expect(user.errors.messages.count).to be 4
        expect(user.errors.attribute_names).to eq %i[name phone address document]
        ...
        expect(user.errors.messages[:address]).to eq ['is too long (maximum is 150 characters)']
        expect(user.errors.messages[:document]).to eq ['is too long (maximum is 16 characters)']
      end
    end
  end
end
